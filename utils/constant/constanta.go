...
const (
	...
	GET_ALL_USERS              = `SELECT id, id_wallet, username, email, fullname, borndate, phone_number, address, created_at, status FROM m_user_account where status='A';`
	GET_ALL_PROVIDERS          = `SELECT id, username, email, fullname, borndate, phone_number, address, created_at, status FROM m_provider_account where status='A';`
	...
	GET_ALL_REVIEWS            = `SELECT id, user_id, asset_id, rating, comment, created_at, status FROM m_review where status='A';`
	...
	DELETEPROVIDER            = `UPDATE m_provider_account SET status="I" where id=?;`
