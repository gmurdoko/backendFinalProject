...
//Payment app
func (s walletRepositoryImpl) Payment(Wallet *models.Wallets) error {
	tx, err := s.db.Begin()
	fmt.Println("PAYMENT")
...
//Receive app
func (s walletRepositoryImpl) Receive(Wallet *models.Wallets) error {
	tx, err := s.db.Begin()
	Wallet.EditedAt = time.Now().Format(`2006-01-02 15:04:05`)
	if err != nil {
		return err
...
func (s walletRepositoryImpl) SelectWalletByID(id string) (*models.Wallets, error) {
	fmt.Println("wallet by ID")
	var wallet = new(models.Wallets)
